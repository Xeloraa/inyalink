...
|---|---|---|---|---|
| `profiles` | table | App profile linked 1:1 to `auth.users`: role (`client` / `professional` / `admin`), display name, locale. **No phone** (lives in Auth only). | Public-facing identity without duplicating auth PII. `role = 'admin'` gates `/admin` API + UI; seeded local admin + `ADMIN_EMAIL` promotion. | Until account deletion (`on delete cascade` from `auth.users`). |
...
| `professionals` | table | Professional application/profile: category, optional `category_other_text` (when category is `other`), bilingual headline/bio, skills, review status/notes, turnaround, min budget (kyat `bigint`), accepting flag, `partner_tier` (fallback / urgent pool), optional `cv_url` (external http(s) link only). | Vetting and matching. Review fields support admin queue. Partner tier is ops designation, not paid boost. `category_other_text` surfaces unmet demand outside named verticals. `cv_url` is a link-out — never identity-document storage. | Until profile/account deletion. Rejected applications remain until account delete unless admin purges. |
...
| `work_links` | table | Outbound profile links: platform (`github`/`behance`/`dribbble`/`website`/`instagram`/`facebook`/`linkedin`/`other`), URL, optional label, `verified_at` (URL resolve check). | Client trust via labelled link-outs. Never scrapes or imports remote content. | Until professional row deleted (cascade) or link removed. |
...
| `engagements` | table | Match/transaction record: brief ↔ professional, status machine, amount, match/decline reasons, `respond_by` (24h accept/decline window), timestamps. | Reputation and completion metrics derive only from here. Decline/expiry triggers top-3 backfill from the interested pool. | Until brief deletion (cascade) or explicit purge policy later. Prefer retain through dispute window. |
| `messages` | table | Engagement thread body + sender. `expires_at` defaults to now + 90 days. | Coordination between client and professional. | **90 days** via column default + `pg_cron` hard delete (`delete-expired-messages`) **and** API hourly retention fallback (`lib/retention.ts`). Free-tier DBs pause after inactivity so cron may not fire while paused. Never soft-delete. |
| `ai_conversations` | table | Signed-in floating-chat session: title (from opening message), path, brief_draft JSON, complete flag. `expires_at` defaults to now + 90 days. | Resume hire conversations; holds business plans/budgets. | **90 days** via column default + `pg_cron` hard delete (`delete-expired-ai-conversations`) **and** API hourly retention fallback. Never soft-delete. Anonymous chat stays in browser sessionStorage only — never uploaded on sign-in. |
...
| `notifications` | table | In-app bell items: recipient `user_id`, `type` (match/engagement/application), deep-link `href`, optional `brief_id` / `engagement_id`, JSON `meta` (brief title / display name for copy), `read_at`. | Alert pros and clients to top-3 selection, proposals, accept/decline, and application review outcomes. Copy is i18n on the client from `type`. | Until account deletion (cascade). Propose 90-day hard delete of read rows once volume justifies a job. |
