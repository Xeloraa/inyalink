...
|---|---|---|---|---|
| `profiles` | table | App profile linked 1:1 to `auth.users`: role, `is_admin` (ops console gate), display name, locale. **No phone** (lives in Auth only). | Public-facing identity for clients and professionals without duplicating auth PII. `is_admin` gates `/admin` API + UI; seeded local admin + `ADMIN_EMAIL` promotion. | Until account deletion (`on delete cascade` from `auth.users`). |
