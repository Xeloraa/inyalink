...
| `professionals` | table | Professional application/profile: category, optional `category_other_text` (when category is `other`), bilingual headline/bio, skills, review status/notes, turnaround, min budget (kyat `bigint`), accepting flag, `partner_tier` (fallback / urgent pool). | Vetting and matching. Review fields support admin queue. Partner tier is ops designation, not paid boost. `category_other_text` surfaces unmet demand outside named verticals. | Until profile/account deletion. Rejected applications remain until account delete unless admin purges. |
