  ...
  return (
    <ul className={className}>
      {visible.map((skill) => (
        <li
          key={skill}
          className="rounded-full border border-line bg-white px-sm py-0.5 text-caption text-ink-700"
        >
          {skill}
        </li>
      ))}
...
export function ProRow({
  ...
  return (
    <article className="group relative rounded-lg border border-line bg-white p-md transition-[transform,box-shadow] duration-base ease-out hover:-translate-y-[2px] hover:shadow-md focus-within:shadow-md motion-reduce:transition-none motion-reduce:hover:translate-y-0 sm:p-lg md:p-xl">
      <span
        aria-hidden
       ...
       * headline link (and desktop View-profile) are the accessible paths.
       */}
      <Link
        to={profilePath}
        aria-hidden
      ...
      <div className="flex gap-md sm:gap-lg">
        ...
        <div className="min-w-0 flex-1">
          ...
          {headline ? (
            <h2 className="mt-0.5 text-body-sm sm:mt-xs sm:text-title">
              <Link
                to={profilePath}
          ...
          {pro.location ? (
            <p className="mt-0.5 hidden items-center gap-xs text-caption text-ink-400 sm:mt-xs sm:flex">
              <PinIcon />
              {pro.location}
            </p>
          ) : null}
        ...
        <div className="relative z-10 hidden shrink-0 flex-col items-end justify-between gap-md self-stretch sm:flex">
          <SaveButton saved={saved} onToggle={onToggleSave} />
          <Link
            to={profilePath}
      ...
      <div className="sm:hidden">
        <p className="no-scrollbar mt-xs overflow-x-auto whitespace-nowrap text-caption">
          {metrics.map((metric, i) => (
        ...
        <SkillChips
          ...
          className="mt-xs flex flex-nowrap gap-xs overflow-hidden"
        />

        {bio ? (
          <p className="mt-xs line-clamp-1 text-caption text-ink-500 [overflow-wrap:anywhere]">
            {bio}
          </p>
        ) : null}
      </div>
